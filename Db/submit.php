...
// Copy of Db/submit.php for servers where folder is lowercase 'db'
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name) || empty($email) || empty($message)) {
        ...
        $resp = ['status' => 'error', 'message' => 'All fields are required'];
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        http_response_code(400);
        $resp = ['status' => 'error', 'message' => 'Invalid email format'];
    ...
    if ($stmt->execute()) {
        ...
        $resp = ['status' => 'ok', 'message' => 'Data sent successfully'];
        echo json_encode($resp, JSON_UNESCAPED_UNICODE);
    } else {
        http_response_code(500);
